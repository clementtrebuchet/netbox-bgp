...
class CommunityTestCase(BaseTestCase):
    ...
    def test_graphql(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            json.loads(response.content)["data"]["community"]["value"],
            self.community1.value,
        )
    ...
    def test_graphql_list(self):
        ...
        response = self.gql_client.post(
            url, json.dumps({"query": query}), content_type="application/json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class PeerGroupTestCase(BaseTestCase):
    ...
    def test_graphql(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            json.loads(response.content)["data"]["peer_group"]["name"],
            self.peer_group.name,
        )
...
class SessionTestCase(BaseTestCase):
    def setUp(self):
        ...
        self.local_as = ASN.objects.create(
            asn=65000, rir=self.rir, description="local_as"
        )
        self.remote_as = ASN.objects.create(
            asn=65001, rir=self.rir, description="remote_as"
        )
    ...
    def test_get_session(self):
        ...
        self.assertEqual(
            ...
            self.session.local_address.address,
        )
        self.assertEqual(
            response.data["remote_address"]["address"],
            self.session.remote_address.address,
        )
    ...
    def test_graphql(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            json.loads(response.content)["data"]["bgp_session"]["name"],
            self.session.name,
        )
